...
## D-030 Show Claude extra usage and Codex credits (2026-09-24)

- Context: the owner wants credit usage on the dashboard when it can be read, and optionally in the menu bar and widget, with or without the limits.
- Rule applied: owner request; the owner approved one key-only probe of the usage endpoint (`probe --claude-oauth-dump-keys`, key paths and types only).
- Decision: (1) Sources. Claude: the usage API's `spend` object, whose amounts are self-describing (`amount_minor`, `currency`, `exponent`) and carry `enabled`; `extra_usage` has the same data with ambiguous units and is not used. Codex: the app-server's `credits` (`hasCredits`, `unlimited`, decimal-string `balance` in credits); rollout credits are not read. Both are parsed leniently from untyped JSON, so an unexpected shape drops only the credits and never the limit windows. Claude credits therefore need the opt-in usage API (D-023). (2) Model: `Credits { enabled, unlimited, used, limit, balance, source, observedAt }` with exact `CreditAmount { minor, exponent, currency }`, stored per source and shown from the newest source that reports them, independent of which source is authoritative for limits. A complete reading without credits clears them; a sparse one keeps them. (3) Dashboard: always shows reported credits under the limits, as "Extra usage: $12.40 of $50.00" with a spend bar or "Credits: 120.50 remaining balance", or a muted "Extra usage is off" / "No Codex credits". (4) New settings `trayCredits` and `widget.credits`: `Off` (default), `WithLimits`, `Only`. A provider with nothing to show (extra usage off, no Codex credits) keeps its limit even with `Only`, so no provider goes blank. The menu bar renders credits as text (`C 41% $12.40`; in Bars style a `C $12.40` title beside the bars; `Only` uses text with the template icon). Widget: an extra value (Pill), an `ex`/`cr` row (Stack; a provider with three rows uses tighter spacing to fit the fixed size) or appended amounts (Mini).
- Why: exact minor units avoid float rounding in money; falling back to limits keeps every provider visible, matching the owner's D-029 choice.
- Limits: the Claude endpoint is undocumented, so the amount's meaning (monthly extra-usage spend) must be confirmed against Claude's own usage page once (HANDOVER); Mini's hover values fit amounts up to about `$99.99` beside both percentages.
- Evidence: parser tests cover the verified `spend` shape and Codex balances, including malformed inputs that keep the windows; merge tests cover cross-source credits and sparse/complete replacement; tray tests cover all three modes and exact formatting; the widget gallery and dashboard were measured in the browser for overflow at their fixed sizes.
